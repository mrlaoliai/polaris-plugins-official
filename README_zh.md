...
**底层驱动**：`@nut-tree-fork/nut-js`  
**亮点**：纯 TypeScript 编写，原生调用操作系统 API，全架构跨平台（macOS / Windows / Linux）
...
**底层驱动**：Node.js 原生 `fs` 模块  
**亮点**：纯 TypeScript 编写，零外部依赖，支持 `POLARISAGI_KB_ALLOWED_DIR` 环境变量进行路径沙盒隔离白名单
...
请确保你的系统上已经安装了 **Node.js** (v18 或更高版本)。现在所有的插件都基于 NPM 自动分发，**无需再手动下载任何二进制文件！**
...
plugins/
  computer_use/
    ...
    .mcp.json                    # MCP 配置（command: npx）
    src/index.ts                 # TypeScript MCP 服务端
    package.json
  browser_use/
