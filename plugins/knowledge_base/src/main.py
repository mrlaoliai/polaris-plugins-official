#!/usr/bin/env python3
"""
PolarisAGI Knowledge Base MCP Server
...
ALLOWED_DIR = os.environ.get("POLARISAGI_KB_ALLOWED_DIR")
...
def handle_list_files(args):
    ...
    if not is_path_allowed(target_path):
        raise PermissionError("path is outside the allowed directory (POLARISAGI_KB_ALLOWED_DIR)")
        ...
        try:
            if method == "initialize":
                _send_result(req_id, {
                    ...
                    "capabilities": {"tools": {}},
                    "serverInfo": {"name": "polarisagi-knowledge-base-mcp", "version": "1.0.0"},
                })
            elif method == "ping":
                _send_result(req_id, {})
