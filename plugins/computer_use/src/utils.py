"""
Platform utilities shared by main.py and adapters.
All keyboard, mouse, clipboard, and accessibility helpers live here.
"""
import json
...
# ---------------------------------------------------------------------------
# Universal keyboard helpers (pynput — sends to frontmost app)
# ---------------------------------------------------------------------------

def press_shortcut(shortcut: str):
    """Press a keyboard shortcut via pynput (goes to the current frontmost app)."""
...
def clipboard_type(text: str):
    """
    Inject text via clipboard (macOS: pbcopy + Cmd+V).
    Handles CJK, emoji, and special characters on all platforms.
    """
    if platform.system() == "Darwin":
        try:
...
def clear_field():
    """Select-all + delete in the focused field (pynput)."""
...
# ---------------------------------------------------------------------------
# macOS AppleScript helpers — process-targeted keyboard ops (bypass focus)
# ---------------------------------------------------------------------------

def osa_send_key(proc_name: str, shortcut: str) -> bool:
    """
    Send a keystroke directly to proc_name via AppleScript.
    The process does NOT need to be frontmost.
    shortcut: e.g. "cmd+f", "enter", "down", "cmd+a"
    """
...
def osa_paste(proc_name: str, text: str) -> bool:
    """Copy text to clipboard and paste into proc_name (Cmd+V, process-targeted)."""
    try:
...
def osa_clear(proc_name: str):
    """Select-all + delete inside proc_name's focused field."""
...
# ---------------------------------------------------------------------------
# macOS App Focus + Window Screenshot helpers
# ---------------------------------------------------------------------------

def ensure_frontmost(proc_name: str, open_name: str, bundle_id: str = "", retries: int = 2) -> bool:
    """
    Ensure the target macOS app is frontmost.
    Re-activates up to `retries` times before giving up.
    Returns True when confirmed frontmost, False if unable.
    Non-Darwin platforms always return True.
    """
    if platform.system() != "Darwin":
        return True
...
def capture_app_window(proc_name: str) -> tuple:
    """
    Capture a screenshot of the target app's largest window only.

    Returns (tmp_path, x_offset, y_offset):
      - tmp_path   : path to the saved PNG (caller must delete when done)
      - x_offset   : window left edge in logical screen coordinates
      - y_offset   : window top  edge in logical screen coordinates

    OCR coordinates from the saved image are relative to the window top-left.
    Add (x_offset, y_offset) to convert them to absolute screen coordinates.

    Falls back to a full-screen capture (offsets = 0, 0) if window bounds are
    ...
    tmp = _tf.mktemp(suffix=".png")
    bounds = get_largest_window_bounds(proc_name)
...
def safe_click(x: int, y: int, proc_name: str, open_name: str, bundle_id: str = "") -> bool:
    """
    Bring the target app to front, then immediately click at absolute screen
    coordinates (x, y).  Returns True if the app was confirmed frontmost before
    the click; False if it could not be brought to front (click is skipped).
    On non-Darwin platforms the frontmost check is skipped.
    """
    if platform.system() == "Darwin":
