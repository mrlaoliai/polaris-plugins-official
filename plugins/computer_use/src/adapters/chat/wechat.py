...
class WeChatAdapter(BaseAdapter):
    ...
    def select_search_result(
        ...
        wait_ocr = float(profile.get("wait_before_ocr", 2.5))
        if wait_ocr > 0:
    ...
    def _retype_search(
        ...
        """
        Re-open WeChat's search box and retype the query.

        Taking a window screenshot hands focus back to the system, which causes
        WeChat to clear its search-input field.  This helper re-activates WeChat,
        presses the search shortcut to focus the search box, clears any leftover
        text, and types contact_name so the result list reappears.
        """
        ...
        # Retype the query character by character.
        for ch in contact_name:
            kb.tap(ch)
            time.sleep(0.04)

        # WeChat search renders in two phases:
        #   Phase 1 (~0-1 s): inline 群聊 dropdown appears immediately
        #   Phase 2 (~1-3 s): full search-results panel with section headers loads
        # We must wait until Phase 2 is stable before clicking, because the
        # coordinates recorded by the earlier OCR correspond to the Phase-2 layout.
        wait_ocr = float(profile.get("wait_before_ocr", 2.5))
        phase1 = min(1.0, wait_ocr * 0.4)
        phase2 = wait_ocr - phase1
        log(f"[4A] Phase-1 wait {phase1:.1f}s (inline results)")
        time.sleep(phase1)
        log(f"[4A] Phase-2 wait {phase2:.1f}s (full search panel)")
        time.sleep(phase2)
...
def _screenshot_window(profile: dict) -> tuple:
    ...
    try:
        if bounds and bounds.get("w", 0) > 50:
            region = {
